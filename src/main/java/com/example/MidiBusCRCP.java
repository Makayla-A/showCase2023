 ...
 * 
 * The MidiBus is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser General Public License as published by
 ...
 * CREDIT: Severin Smith; Professor Courtney Brown (author(s) of: public MidiBusCRCP(String out_device_name); public MidiBusCRCP(int out_device_num); private void init() ;
 * static MidiDevice.Info[] availableOutputsMidiDeviceInfo(); static public String[] availableOutputs(); static public void listDevices(); synchronized boolean addOutput(MidiDevice.Info device_info);
 * public boolean addOutput(String device_name); public boolean addOutput(int device_num); public void sendMessage(byte[] data); public void sendMessage(int status); public void sendMessage(int status, int data);
 * public void sendMessage(int status, int data1, int data2); public void sendMessage(int command, int channel, int data1, int data2); public synchronized void sendMessage(MidiMessage message);
 * public void sendNoteOn(int channel, int pitch, int velocity); public void sendNoteOff(int channel, int pitch, int velocity); int constrain(int value, int min, int max); private class OutputDeviceContainer)
 * CLASS: MidiBusCRCP; Sends midi notes to an external player/midi channel -- does not handle input, listening, etc.
 * 
 */
